...
model_path = os.path.join(current_dir, '..', 'models', 'multi_nlp_model.pkl')
...
def predict_sentiment(tweet_request: TweetRequest) -> dict:
    """
    Predict the sentiment of a tweet using the pre-trained model.

    Args:
        tweet_request (TweetRequest): The request object containing the tweet text.

    Returns:
        dict: The predicted sentiment and confidence.
    """
    if model is None:
        return {"error": "Model not loaded"}

    try:
        # Extract the tweet text from the request
        tweet_text = tweet_request.tweet

        # Create DataFrame in the format expected by the pipeline
        tweet_df = pd.DataFrame({'tweet': [tweet_text]})

        # Get prediction and probabilities
        prediction = model.predict(tweet_df)[0]
        probabilities = model.predict_proba(tweet_df)[0]
